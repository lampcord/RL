...
namespace MCTSRGDAgentNS
{
	...
	inline void MCTSRGDAgent<TGameRules, TNodeStorage, TNodeID, TPosition, TMoveType>::get_root_choice_map(unordered_map<TMoveType, float>& choice_map)
	{
		...
		for (auto child_ndx = 0; child_ndx < root_node->num_children.load(); child_ndx++)
		{
			...
			choice_map[child->move_to_reach_position] += child->num_visits.load() / root_node->num_visits.load();
		}
	}
	...
	inline TNodeID MCTSRGDAgent<TGameRules, TNodeStorage, TNodeID, TPosition, TMoveType>::expand(TNodeID node_id)
	{
		...
		if (node->remaining_moves_mask == 0 && node->num_children == 0)
		{
			...
			node->remaining_moves_mask.store(remaining_moves_mask);
			auto num_children = get_num_moves(node->remaining_moves_mask.load());
			if (num_children > 0)
			{
				auto expanded = node_storage.reserve_child_nodes(node_id, num_children);
				if (!expanded)
				{
					node->remaining_moves_mask.store(0u);
					return node_id;
				}
			}
		}
		...
		auto move = get_first_move(node->remaining_moves_mask.load());
	...
	inline TNodeID MCTSRGDAgent<TGameRules, TNodeStorage, TNodeID, TPosition, TMoveType>::best_child(TNodeID node_id)
	{
		...
		for (auto child_ndx = 1; child_ndx < node->num_children.load(); child_ndx++)
		{
	...
	*/
	...
	inline TNodeID MCTSRGDAgent<TGameRules, TNodeStorage, TNodeID, TPosition, TMoveType>::best_child_fast(TNodeID node_id)
	{
		...
		auto exploration_numerator = _c * sqrt(log(node->num_visits.load()));
		...
		for (auto child_ndx = 0; child_ndx < node->num_children.load(); child_ndx++)
		{
			...
			auto exploration = exploration_numerator / sqrt(child->num_visits.load());
			auto exploitation = child->num_wins.load() / child->num_visits.load();
	...
	inline TNodeID MCTSRGDAgent<TGameRules, TNodeStorage, TNodeID, TPosition, TMoveType>::most_visited(TNodeID node_id)
	{
		...
		for (auto child_ndx = 0; child_ndx < node->num_children.load(); child_ndx++)
		{
			...
			auto score = child->num_visits.load();
	...
	*/
	...
	inline float MCTSRGDAgent<TGameRules, TNodeStorage, TNodeID, TPosition, TMoveType>::ucb(TNodeID node_id, float c)
	{
		...
		auto exploration = c * sqrt(log(parent->num_visits.load()) / node->num_visits.load());
