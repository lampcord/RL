...
def nature_cnn(observation_space, depths=(32, 64, 64), final_layer=512):
    ...
    cnn = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Conv2d(depths[0], depths[1], kernel_size=8, stride=2),
        nn.ReLU(),
        nn.Conv2d(depths[1], depths[2], kernel_size=8, stride=1),
        nn.ReLU(),
        nn.Flatten())
...
for step in itertools.count():
    ...
    if isinstance(obses[0], PytorchLazyFrames):
        act_obs = np.stack([o.get_frames() for o in obses])
        actions = online_net.act(act_obs, epsilon)
    else:
