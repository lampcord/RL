...
def draw_node(screen, cell_size, offset, pos, turn, possible_moves, cell_padding=5, win_set=None):
    for row in range(BOARD_ROWS):
        for col in range(BOARD_COLS):
            ...
            if win_set and (col, row) in win_set:
                pygame.draw.circle(screen, YELLOW, (xpos + cell_size // 2, ypos + cell_size // 2), cell_size // 2 - cell_padding)
                pygame.draw.circle(screen, cell_color, (xpos + cell_size // 2, ypos + cell_size // 2), cell_size // 2 - 2 * cell_padding)
            else:
...
def draw_board(screen, pos, turn, possible_moves, message, win_set=None):
    screen.fill(GREY)
    ...
    draw_node(screen, CELL_SIZE, (xoffset, yoffset), pos, turn, possible_moves, win_set=win_set)
