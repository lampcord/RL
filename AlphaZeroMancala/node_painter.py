...
class NodePainter:
    ...
    def paint(self, label='', selected_node=None, result=None):
        running = True
        ...
        while running:
            ...
            draw_node(self.screen, self.board, self.node, selected_node, self.font, result=result)
            draw_tree(self.screen, self.board, self.node, selected_node, self.font, 3)
            pygame.display.flip()
...
def draw_node(screen, board, node, selected_node, font, result=None):
    node.render_node(screen, board, font, node == selected_node, result=result)
...
def draw_tree(screen, board, node, selected_node, font, max_depth=2):
    if max_depth <= 0:
        return
    for i, child in enumerate(node.children):
        ...
        draw_node(screen, board, child, selected_node, font)
        draw_tree(screen, board, child, selected_node, font, max_depth - 1)
