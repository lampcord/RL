...
if __name__ == "__main__":
    ...
    score_to_load = '19.19'
    filename = 'models/' + params['env_name'] + "_" + str(score_to_load) + "-best.dat"
    ...
    while not done:
        env.render()
        state_v = torch.tensor(np.array([state.__array__(dtype=np.float32)], copy=False))
        q_vals = net.qvals(state_v).detach().numpy()
